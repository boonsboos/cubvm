...
pub fn generate(tokens: Vec<TokenKind>) -> Result<Code, Error> {
    ...
    let mut buf: Vec<u8> = vec![0xB0u8];
    let mut i: usize = 1; // SOF written above
    while i < tokens.len() {
        ...
        match tokens[i] {
            ...
            TokenKind::Newline => line_count += 1,
            TokenKind::EOF => break,
            _ => {},
        }

        // simple optimization by removing moves that cancel each other out
        if tokens[i+1] == tokens[i].opposite() {
            buf.pop();
        }
